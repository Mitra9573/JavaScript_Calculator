...
// UPDATED DISPLAY FUNCTION (shows operator)
const updateDisplay = () => {
  if (currentOperator && !awaitingSecondOperand && !secondOperand) {
    display.value = `${firstOperand} ${currentOperator}`;
  } 
  else if (currentOperator && awaitingSecondOperand) {
    display.value = `${firstOperand} ${currentOperator} ${secondOperand}`;
  } 
  else {
    display.value = firstOperand || "0";
  }
};
